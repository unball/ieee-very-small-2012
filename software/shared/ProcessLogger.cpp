...
ProcessLogger::~ProcessLogger ()
{
	...
	{
#ifdef _WIN32
		fprintf (stderr, "%s\t%s\t%s\n", logPrefix_.c_str (), logMsg.c_str (), logPostfix.c_str ());
#elif __unix__
...
ostringstream & ProcessLogger::getLogStream (LogLevel level)
{
	...
	*/
	...
	ss_prefix << now.time_of_day ();
	logPrefix_ = ss_prefix.str ();
	logPrefix_ += "\t";
...
string ProcessLogger::getLogVersion (const string & logDir, const string & procName)
{
	...
	if (bfs::exists (logDir))
	{
		...
		for (bfs::directory_iterator itr (logDir); itr != end; ++itr)
		{
			...
			if (!boost::regex_match (itr->path ().string (), processLogMatch, processLogFilter))
			{
				continue;
			}
			LOG (LEVEL_DEBUG) << "process log found: " << itr->path ().string ();
			try
			{
